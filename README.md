# FAP Aponti - DevOps Learning Journey 🚀

Welcome to my DevOps repository! This space is dedicated to documenting my learning journey, storing hands-on lab configurations, and tracking my progress as I master DevOps practices, tools, and culture.

## 📌 Repository Purpose
* **Track Progress:** Document daily learning, exercises, and course challenges.
* **Code Storage:** Centralize Infrastructure as Code (IaC) templates and CI/CD pipelines.
* **Reference Guide:** Act as a personal cheat sheet for real-world projects.

## 🛠️ Technologies & Tools
* **Skills:** Soft Skills and Hard Skills
* **Version Control:** Git, GitHub
* **Linux & Scripting:** Bash, Python
* **Containers & Orchestration:** Docker, Kubernetes
* **Infrastructure as Code:** Terraform, Ansible
* **CI/CD Pipelines:** GitHub Actions, Jenkins
* **Cloud Platforms:** AWS / Azure / GCP
* **Monitoring & Logging:** Prometheus, Grafana, ELK Stack



## 📈 Current Goals
1. Deepen knowledge in Kubernetes cluster administration.
2. Integrate security scanning (SAST/DAST) into CI/CD pipelines (DevSecOps).
3. Implement centralized logging and real-time monitoring solutions.

---
*“Continuous improvement is better than delayed perfection.”*
--------------

